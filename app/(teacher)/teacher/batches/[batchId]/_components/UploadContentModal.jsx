...
// Import UploadThing helpers from our local util so we can easily control
// the API URL and add typings later if needed.
import { UploadDropzone } from "@/utils/uploadthing";
...
export default function UploadContentSheet({ isOpen, onClose, batchId, subjects: initialSubjects }) {
    ...
    const [uploadedPdfUrl, setUploadedPdfUrl] = useState("");
    const [pdfFileName, setPdfFileName] = useState("");
    const [pdfUploadProgress, setPdfUploadProgress] = useState(0); // percent 0-100
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (activeTab === "video") {
                if (!selectedFile) {
                    toast.error("Please select a video file", { id: toastId });
                    return;
                }
                setIsUploading(true);
                const finalVideoId = await uploadToVdoCipher(selectedFile, title);
                await submitLesson({ title, description, playbackId: finalVideoId, isFreePreview, subjectId, batchId });
                toast.success("Video lecture published!", { id: toastId });
            } else {
                if (!uploadedPdfUrl) {
                    toast.error("Please upload the PDF file first", { id: toastId });
                    return;
                }
                await submitMaterial({
                    title,
                ...
                toast.success("Study material published!", { id: toastId });
            }
            resetAndClose();
        } catch (error) {
            console.error("Publish error", error);
            toast.error(error?.message ? `Publish failed: ${error.message}` : "Failed to publish content. See console.", { id: toastId });
        } finally {
            setIsUploading(false);
        }
    };
        ...
        <Sheet open={isOpen} onOpenChange={resetAndClose}>
            <SheetContent className="bg-zinc-950 border-l border-white/10 text-white sm:max-w-xl w-[90vw] p-0 flex flex-col">
                <SheetHeader className="p-6 border-b border-white/10 bg-white/5">
                    ...
                    <SheetDescription className="text-zinc-400">
                        Upload DRM videos or physical PDF notes for your students.
                    </SheetDescription>
                </SheetHeader>
                ...
                <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-6 space-y-8">
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        <TabsList className="grid w-full grid-cols-2 bg-black/40 border border-white/10 rounded-xl p-1 mb-8">
                            <TabsTrigger value="video" className="rounded-lg data-[state=active]:bg-emerald-600 data-[state=active]:text-white text-zinc-400 transition-all">
                                <Video className="h-4 w-4 mr-2" /> Encrypted Video
                            </TabsTrigger>
                            <TabsTrigger value="pdf" className="rounded-lg data-[state=active]:bg-blue-600 data-[state=active]:text-white text-zinc-400 transition-all">
                                <FileText className="h-4 w-4 mr-2" /> Physical PDF
                            </TabsTrigger>
                        </TabsList>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label className="text-zinc-300">Content Title</Label>
                                    <Input name="title" required placeholder="e.g., Chapter 1: Introduction" className="bg-black/40 border-white/10 focus-visible:ring-emerald-500 h-11" disabled={isUploading || isSavingPdf} />
                                </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <Label className="text-zinc-300">Subject</Label>
                                        {!isCreatingSubject && (
                                            <button type="button" onClick={() => setIsCreatingSubject(true)} className="text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
                                                + Add New Subject
                                            </button>
                                        )}
                                    </div>
                                    ...
                                    {isCreatingSubject ? (
                                        <div className="flex items-center gap-2">
                                            <Input autoFocus placeholder="New subject name..." value={newSubjectName} onChange={(e) => setNewSubjectName(e.target.value)} className="bg-black/40 border-emerald-500/30 h-11" disabled={isSavingSubject} />
                                            <Button type="button" onClick={handleCreateSubject} disabled={isSavingSubject} className="bg-emerald-600 h-11 px-3">
                                                {isSavingSubject ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                                            </Button>
                                            <Button type="button" variant="ghost" onClick={() => setIsCreatingSubject(false)} className="h-11 px-3 text-zinc-400"><X className="h-4 w-4" /></Button>
                                        </div>
                                    ) : (
                                        <Select value={subjectId} onValueChange={setSubjectId} required disabled={isUploading || isSavingPdf}>
                                            <SelectTrigger className="bg-black/40 border-white/10 h-11"><SelectValue placeholder="Select Subject" /></SelectTrigger>
                                            <SelectContent className="bg-zinc-900 text-white border-white/10">
                                                {localSubjects.map((sub) => <SelectItem key={sub.id} value={sub.id}>{sub.name}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea name="description" rows={3} placeholder="Brief summary..." className="bg-black/40 border-white/10 resize-none" disabled={isUploading || isSavingPdf} />
                                </div>
                            </div>
                            ...
                            {activeTab === "video" && (
                                <div className="border border-emerald-500/20 rounded-xl p-6 bg-emerald-500/5 relative overflow-hidden group">
                                    <input type="file" accept="video/*" className="hidden" ref={fileInputRef} onChange={handleFileChange} disabled={isUploading} />
                                    {!selectedFile ? (
                                        <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-emerald-500/30 hover:border-emerald-500 rounded-xl p-8 bg-black/40 text-center cursor-pointer">
                                            <UploadCloud className="h-8 w-8 text-emerald-500 mx-auto mb-4" />
                                            <p className="text-sm font-semibold text-white">Select Video File</p>
                                        </div>
                                    ) : (
                                        <div className="p-4 bg-black/60 border border-emerald-500/30 rounded-xl flex items-center justify-between">
                                            <div className="flex items-center gap-3 overflow-hidden">
                                            ...
                                            {!isUploading && <X className="h-4 w-4 text-zinc-400 cursor-pointer" onClick={() => setSelectedFile(null)} />}
                                        </div>
                                    )}
                                    {isUploading && (
                                        <div className="mt-4 space-y-2">
                                            <div className="flex justify-between text-xs font-medium text-emerald-400">
                                                <span>Uploading to DRM Vault...</span>
                                                <span>{uploadProgress}%</span>
                                            </div>
                                            <div className="w-full bg-black/60 h-2 rounded-full overflow-hidden"><div className="bg-emerald-500 h-2 transition-all" style={{ width: `${uploadProgress}%` }} /></div>
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === "pdf" && (
                                <div className="border border-blue-500/20 rounded-xl p-6 bg-blue-500/5">
                                    <div className="flex items-center gap-2 mb-4 text-blue-400"><FileText className="h-5 w-5" /><h4 className="font-semibold">PDF Document Upload</h4></div>
                                    {!uploadedPdfUrl ? (
                                        <div className="bg-black/40 rounded-xl border border-white/10 overflow-hidden">
                                            <UploadDropzone
                                            endpoint="pdfUploader"
                                            config={{ mode: "auto" }}  // start upload immediately on file select
                                            onUploadBegin={() => {
                                                setPdfUploadProgress(0);
                                            }}
                                            onUploadProgress={(p) => setPdfUploadProgress(p)}
                                            onClientUploadComplete={(res) => {
                                                console.log("UploadThing complete result", res);
                                                if (Array.isArray(res) && res.length > 0) {
                                                    const file = res[0];
                                                    const url = file.url || file.fileUrl || file.ufsUrl || file.appUrl || "";
                                                    setUploadedPdfUrl(url);
                                                    setPdfFileName(file.name || file.fileName || "uploaded.pdf");
                                                    setPdfUploadProgress(100);
                                                    toast.success("PDF uploaded to cloud!");
                                                } else {
                                                    toast.error("Unexpected upload response", { duration: 4000 });
                                                }
                                            }}
                                            onUploadError={(err) => {
                                            ...
                                            className="ut-button:bg-blue-600 ut-button:ut-readying:bg-blue-600/50 ut-label:text-blue-400 border-dashed border-blue-500/30"
                                        />
                                        </div>
                                    ) : (
                                        <div className="p-4 bg-black/60 border border-blue-500/30 rounded-xl flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                <FileCheck className="h-5 w-5 text-blue-400" />
                                                <p className="text-sm text-white truncate max-w-50">{pdfFileName}</p>
                                            </div>
                                            <Button type="button" variant="ghost" size="icon" onClick={() => { setUploadedPdfUrl(""); setPdfFileName(""); setPdfUploadProgress(0); }} className="h-8 w-8 text-zinc-400 hover:text-red-400">
                                                <X className="h-4 w-4" />
                                            </Button>
                                        </div>
                                    )}

                                    {/* show upload progress when the dropzone is actively uploading */}
                                    {pdfUploadProgress > 0 && pdfUploadProgress < 100 && (
                                        <div className="mt-2">
                                            ...
                                            <div className="w-full bg-black/60 rounded-full h-2 overflow-hidden border border-white/5">
                                                <div className="bg-blue-500 h-2 transition-all duration-300" style={{ width: `${pdfUploadProgress}%` }}></div>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-xl">
                                <div><Label className="text-white">Free Demo</Label><p className="text-xs text-zinc-400">Allow preview for unpaid users.</p></div>
                                <Switch checked={isFreePreview} onCheckedChange={setIsFreePreview} className="data-[state=checked]:bg-emerald-500" />
                            </div>
                        </div>
                    </Tabs>

                    <div className="pt-6 border-t border-white/10 flex justify-end gap-4 sticky bottom-0 bg-zinc-950 pb-2">
                        <Button type="button" variant="outline" onClick={resetAndClose} disabled={isUploading || isSavingPdf} className="border-white/10">Cancel</Button>
                        <Button type="submit" disabled={
                            ...
                            } className={activeTab === 'video' ? "bg-emerald-600 hover:bg-emerald-700 w-full md:w-auto" : "bg-blue-600 hover:bg-blue-700 w-full md:w-auto"}>
                            {isUploading || isSavingPdf ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Publishing...</> : "Securely Publish"}
                        </Button>
                    </div>
                </form>
            </SheetContent>
